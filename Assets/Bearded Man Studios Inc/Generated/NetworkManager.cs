...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] SpacecraftNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is SpacecraftNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (SpacecraftNetworkObject.Length > 0 && SpacecraftNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(SpacecraftNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<SpacecraftBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateSpacecraft instead, its shorter and easier to type out ;)")]
		public SpacecraftBehavior InstantiateSpacecraftNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(SpacecraftNetworkObject[index]);
			var netBehavior = go.GetComponent<SpacecraftBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<SpacecraftBehavior>().networkObject = (SpacecraftNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Spacecraft
		/// </summary>
		/// <returns>
		/// A local instance of SpacecraftBehavior
		/// </returns>
		/// <param name="index">The index of the Spacecraft prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public SpacecraftBehavior InstantiateSpacecraft(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (SpacecraftNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: Spacecraft. Add them at the NetworkManager prefab.");
				return null;
			}
